...
const googleAuth = (req, res) => {
    ...
    // P5.2: Sanitize name from Google
    const safeGoogleName = (typeof name === "string" && name.trim().length > 0)
    ...
    db.query(
        ...
        (err, rows) => {
            ...
            if (rows.length > 0) {
                ...
                `;
                db.query(insertSql, [safeGoogleName, email.trim().toLowerCase(), google_id], (err2, result) => {
                    if (err2) {
                    ...
                    res.json({
                        success: true,
                        token,
                        user: { id: result.insertId, name: safeGoogleName, email: email.trim().toLowerCase() }
                    });
                });
            }
        }
