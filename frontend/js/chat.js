...
(function () {
  ...
  // Used for "Last seen Xm ago" and "Delivered/Seen Xm ago".
  // Deliberately coarse (matches WhatsApp-style granularity) rather
  // than exact seconds, since exact-second counters churn the DOM
  // every tick for no real benefit to the reader.
  function humanizeTimeAgo(isoOrDate) {
    ...
    // Beyond a week, fall back to an actual date — "last seen 3 weeks ago"
    // stops being a useful number for either party.
  ...
  function renderPresence() {
    ...
    if (partnerPresence.online) {
      ...
      elHeaderSub.innerHTML = `${escapeHTML(roleLabel)} · Last seen ${humanizeTimeAgo(partnerPresence.last_seen)}`;
    } else {
      elHeaderSub.textContent = roleLabel;
    }
  }
