...
function handleLogin(email, hashedPassword) {
    ...
    fetch("https://dhas-production.up.railway.app/login", {
        method: "POST",
...
function handleRegister(name, email, hashedPassword) {
    ...
    fetch("https://dhas-production.up.railway.app/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        // ✅ Only the hash is sent — plain text NEVER leaves the browser
...
async function handleGoogleAuth(name, email, googleId) {
    try {
        const res = await fetch("https://dhas-production.up.railway.app/auth/google", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
